...
function monthNavBlock() {
    ...
    return `
        <div class="month-navigation-inline">
            <button class="month-nav-arrow" data-month-prev title="Mês anterior"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg></button>
            <div class="month-display-inline"><span id="currentMonth">${MONTH_NAMES[ref.getMonth()]} ${ref.getFullYear()}</span></div>
            <button class="month-nav-arrow" data-month-next title="Próximo mês"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg></button>
        </div>
        <button class="calendar-btn" data-calendar-btn title="Selecionar mês"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></button>
    `;
}
...
function openProfissionalModal(editId) {
    ...
    `).join('') : '<p style="color:var(--text-secondary);font-size:0.85rem;">Nenhuma observação registrada.</p>';
            ...
            <div class="tabs-container">
                ...
                <div class="tab-content" data-pane="obs">
                    ...
                    ${p ? `<div class="nova-observacao" style="margin-top:1rem;">
                        <label style="font-size:0.8rem;">Nova observação</label>
                        <textarea id="novaObsProf" rows="3" placeholder="Escreva uma observação..."></textarea>
                        <button class="btn-add-obs small" id="addObsProfBtn">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                            Adicionar
                        </button>
                    </div>` : '<p style="color:var(--text-secondary);font-size:0.85rem;">Salve o profissional para adicionar observações.</p>'}
                </div>
            </div>
        `,
        actions: [
...
function renderAtendimentos(root) {
    ...
    root.innerHTML = `
        ${moduleHead('Controle de Atendimento','Acompanhe os atendimentos do mês. As atualizações vêm do módulo Agendamentos.')}

        <div class="dashboard-grid">
            <div class="stat-card"><div class="stat-icon stat-icon-default"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></div><div class="stat-content"><div class="stat-value">${total}</div><div class="stat-label">Total no período</div></div></div>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th>ID</th><th>Data</th><th>Hora</th><th>Cliente</th><th>Serviço</th><th>Profissional</th><th>Status</th>
...
function renderAgendamentos(root) {
    ...
    root.innerHTML = `
        ${moduleHead('Agendamentos','Crie e atualize agendamentos. Cancelamentos são feitos pelo Controle de Atendimento.',
            ...
            <div class="tabs-container">
                ...
                <div class="tab-content active" data-pane="info">
                    <form id="formAg" class="form-grid">
                        ...
                        <div class="form-group full"><label>Horário <span class="req">*</span></label>
                ...
                <div class="tab-content" data-pane="obs">
                    <div class="info-section">
                        ...
                        ${obsHTML}
                        <div class="nova-observacao" style="margin-top:1rem;">
                            <label for="novaObsAg" style="font-size:0.8rem;">Nova observação</label>
                            <textarea id="novaObsAg" rows="3" placeholder="Escreva uma observação..."></textarea>
        ...
        actions: [
            ...
            { label: a ? 'Atualizar' : 'Salvar', class: 'success', close: false, onClick: async (ov, close) => {
                ...
                if (!clienteEncontrado) {
                    const querCadastrar = await confirmDialog('Você deseja cadastrar este cliente?', { confirmLabel: 'Sim', cancelLabel: 'Não' });
                    if (querCadastrar) {
...
function renderCanais(root) {
    ...
    root.innerHTML = `
        ${moduleHead('Canais de Atendimento','Solicitações recebidas por canais. Listadas da mais antiga para a mais recente.',
            ...
            <div style="overflow-x: auto;">
                <table>
                    <thead>
                        <tr>
                            <th style="width:44px;text-align:center;">✓</th>
                            <th>Entrada</th><th>Concluído</th><th>Cliente</th><th>Canal</th><th>Assunto</th>
                            <th style="text-align:right;">Ações</th>
                        </tr>
                    </thead>
...
function confirmarConclusao(id, checkbox) {
    confirmDialog('Você confirma o atendimento para este contato?', { confirmLabel: 'Sim', cancelLabel: 'Não' })
    .then(confirmado => {
...
function renderClientes(root) {
    root.innerHTML = `
        ${moduleHead('Clientes','Cadastro, contato e histórico de relacionamento.',`<button class="btn-new-order-header" id="btnNovoCli">Novo cliente</button>`)}
                ...
                <div class="tab-content" data-pane="obs">
                    <div class="info-section">
                        ...
                        ${c ? `<div class="nova-observacao" style="margin-top:1rem;">
                            <label for="novaObsCli" style="font-size:0.8rem;">Nova observação</label>
                            <textarea id="novaObsCli" rows="3" placeholder="Escreva uma observação..."></textarea>
                            <button type="button" class="btn-add-obs small" id="addObsCliBtn">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                                Adicionar
                            </button>
                        </div>` : '<p style="color:var(--text-secondary);font-size:0.85rem;">Salve o cliente para adicionar observações.</p>'}
                    </div>
                </div>
            </div>
        `,
...
function renderHistorico(root) {
    ...
    root.innerHTML = `
        ${moduleHead('Histórico de Atendimento','Atendimentos encerrados no mês para clientes cadastrados.')}
        ...
        <div class="search-bar-wrapper">
            <div class="search-bar">
                ...
                <div class="search-bar-filters">
                    ...
                    ${monthNavBlock()}
                    <button class="calendar-btn" id="hiPdfBtn" title="Gerar PDF do mês">${ICON_PDF}</button>
                </div>
            </div>
        </div>
    ...
    const render = () => {
        ...
        if (!list.length) {
            tbody.innerHTML = `<tr><td colspan="8">${emptyState('Sem histórico','Nenhum atendimento encerrado com estes filtros.')}</td></tr>`;
            return;
        }
        tbody.innerHTML = list.map(a => {
            const codigo = codigoDoAtendimento(a) || '—';
...
function renderRelatorios(root) {
    ...
    root.innerHTML = `
        ...
        <div class="dashboard-grid">
            ...
            <div class="stat-card"><div class="stat-icon stat-icon-success"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg></div><div class="stat-content"><div class="stat-value stat-value-success">${atendidos}</div><div class="stat-label">Concluídos</div></div></div>
            <div class="stat-card"><div class="stat-icon stat-icon-danger"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg></div><div class="stat-content"><div class="stat-value stat-value-danger">${cancelados}</div><div class="stat-label">Cancelados</div></div></div>
        ...
        <div class="card" style="padding:1.5rem;"><div class="info-section"><h4>Serviços mais realizados</h4><div class="report-bars">${Object.entries(porServico).sort((a,b) => b[1] - a[1]).map(([s, v]) => bar(s, v, maxServico)).join('') || '<span style="color:var(--text-secondary);font-size:0.85rem;">Sem dados.</span>'}</div></div></div>
    `;
}
